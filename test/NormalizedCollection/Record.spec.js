...
describe('Record', function() {
  describe('#constructor', function() {
    it('should inherit AbstractRecord', function() {
      var mgr = makePathMgr();
      var rec = new Record(mgr, makeFieldMap(mgr));
  ...
  describe('#child', function() {
    ...
    it('should pick the correct path to descend from', function() {
      ...
      expect(child.getPathMgr().first().parent().url()).toBe(mgr.getPath('p2').url());
    });
    ...
    it('should use the primary/first path if none matches the field name', function() {
      ...
      expect(child.getPathMgr().count()).toBe(1);
      expect(child.getPathMgr().first().parent().name()).toBe('p1');
    });
